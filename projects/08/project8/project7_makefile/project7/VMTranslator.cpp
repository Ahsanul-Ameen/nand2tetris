...
class CodeWriter {
	...
	void generatePushStatic(const string& i) {
		...
		string variable = output_file_name + "." + i;
	...
	void writeGoto (const string& label) {
		...
		string code = "// goto label (unconditional jump) ... \n"; 
		code +=	"@" + label + "\n";
		code += "0;JMP\n";
		out << code<< endl;
	}
	...
	void writeIf (const string& label) {
		...
		string code = "// if-goto label (unconditional jump) ... \n"; 
		code += "@SP\n";
		code += "M = M-1\n"; // --SP
		code += "A = M\n";	// A = M
		code += "D = M\n"; 	// D = *SP
	...
	void writeCall (const string& functionName, const int& numArgs) {
		...
		static int i = 1; 
		// file name should be where the caller resides
		// functionName should be the called function
		//string ret_addr = input_file_name + "." + current_function + "$ret." + to_string(i);
	...
	void writeFunction (const string& functionName, const int& numLocals) {
		//cout << functionName << " " << numLocals << endl;
...
int main(int argc, char** argv) {
	...
	for(int i = 1; i < argc; ++i) {
		...
		if (is_directory(file_path)) { 
			string output_file = path_string + "/" + (string)path(file_path.filename()) + ".asm";
			CodeWriter writer(output_file);
			int file_no = 0;
			for (const auto& entry : directory_iterator(file_path)) {
				if (entry.is_regular_file() && entry.path().extension() == ".vm") {
					string input_file = (string)entry.path();
					++file_no;
					//cout << entry.path().filename() << endl;
					//cout << "File " << file_no << " is currently processing" << endl;
			...
			string output_file = (string)file_path.parent_path() + "/" + (string)file_path.stem() + ".asm";

			Parser parser(path_string);
			CodeWriter writer(output_file);
			
			generateCode(parser, writer);
		} else cerr << " ----- @ Error processing files/directories @ ----- " << endl;
		*/
	}
